import React from 'react'
import { FiArrowUpRight } from 'react-icons/fi'
import grain_bg from "../assets/Images/grain_background.jpg"

const Contact = ({toggledisplay}) => {
  
  return (
    <div className='py-16 pt-14 lg:py-24 lg:pt-20' id="Contact">
      <div className='container'>
        <div className='bg-gradient-to-r from-emerald-300 to-sky-400 text-gray-900 py-8 px-10 rounded-3xl text-center
         md:text-left relative overflow-hidden z-0'>
          <div className='absolute inset-0 opacity-5 -z-10' style={{ backgroundImage: `url(${grain_bg})` }}></div>
          <div className='flex flex-col md:flex-row gap-8 md:gap-16 items-center'>
            <div>
              <h2 style={{ fontFamily: "'Calistoga', serif" }} className='text-2xl md:text-3xl'>Let's create something amazing together.</h2>
              <p className='text-sm mt-2 md:text-base'>
                Ready to bring your next project to life? Let's connect
                ans discuss how I can help you achieve your goals.
              </p>
            </div>
            <div>
              <button className='text-white bg-gray-900 inline-flex items-center px-6 h-12 rounded-xl gap-2 mt-6 w-max border border-gray-950
              cursor-pointer
              '
               onClick={toggledisplay}
              >
                <span className='font-semibold'>Contact Me</span>
                <FiArrowUpRight className='size-4 ' />
              </button>
            </div>
          </div>
        </div>


      </div>
    </div>
  )
}
export default Contact